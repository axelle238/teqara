<div class="bg-slate-50 min-h-screen py-12 font-sans antialiased text-slate-900">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="space-y-6">
            ...
            <div class="group relative bg-white rounded-[2rem] p-6 border transition-all duration-300 hover:shadow-2xl hover:shadow-indigo-500/10 hover:-translate-y-1 animate-fade-in-up {{ $notif['dibaca'] ? 'border-slate-100 opacity-90' : 'border-indigo-100 ring-4 ring-indigo-500/5 z-10' }}">
                ...
                <div class="flex items-start gap-6">
                    <!-- Icon Box -->
                    <div class="w-16 h-16 rounded-2xl flex items-center justify-center text-2xl shadow-sm shrink-0 transition-transform group-hover:scale-110
                        {{ $notif['warna'] == 'blue' ? 'bg-blue-50 text-blue-500 group-hover:bg-blue-100' : '' }}
                        {{ $notif['warna'] == 'yellow' ? 'bg-amber-50 text-amber-500 group-hover:bg-amber-100' : '' }}
                        {{ $notif['warna'] == 'indigo' ? 'bg-indigo-50 text-indigo-500 group-hover:bg-indigo-100' : '' }}
                        {{ $notif['warna'] == 'emerald' ? 'bg-emerald-50 text-emerald-500 group-hover:bg-emerald-100' : '' }}
                    ">
                        {{ $notif['icon'] }}
                    </div>
