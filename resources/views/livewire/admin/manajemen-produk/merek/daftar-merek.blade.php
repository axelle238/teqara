<div class="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <a href="{{ route('admin.merek.tambah') }}" wire:navigate class="flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white px-6 py-3 rounded-xl text-xs font-bold uppercase tracking-widest shadow-lg shadow-emerald-600/20 transition-all active:scale-95">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-6">
        ...
        <div class="bg-white rounded-[24px] p-6 border border-slate-100 shadow-sm hover:shadow-xl hover:border-emerald-100 transition-all group text-center relative">
            <div class="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity flex gap-2">
                <a href="{{ route('admin.merek.edit', $m->id) }}" wire:navigate class="p-2 bg-slate-50 text-indigo-600 rounded-lg hover:bg-indigo-50 transition-colors">
